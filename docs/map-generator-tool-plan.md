...
0~3단계를 구현하고 에디터에서 실제로 검증했다. **`BackroomsMapGenerator` 와
`TestRoomMap` 은 지우지 않았고, `SampleScene`/`MultiplayerTest` 도 아직 그대로다** —
계획한 대로다.

| 검증 | 결과 |
|---|---|
| `test-room` 파리티 | 박스 10개 값 일치 |
| `backrooms` 파리티 | **박스 736개 + 격자 2450셀(base64 문자열까지) + 스폰 8개 + 격자 원점/셀 크기/층수 일치** = 맵 해시 동일 |
| `MapGeneratorParityTests` | 7/7 통과 (에디터에서 실행) |
| 어긋남 감지 | 이동·회전·삭제 모두 잡음, 갓 구운 레벨은 오탐 없음 |
| 프리팹 크기 (R1) | Backrooms 조각 1371개 → 씬 오브젝트 **739개**, 프리팹 **2.3MB** |
| 컴파일 | `Assembly-CSharp`, `Assembly-CSharp-Editor` 둘 다 오류 0 |

**R1 판정.** 콜리전 없는 조각 635개(천장 타일·조명 패널)를 표면마다 메시 하나로
합쳐 오브젝트를 2개로 줄였다. 남은 2.3MB 는 거의 전부 벽 736개이고, 이것은 **런타임
생성기가 지금도 만드는 것과 같은 수**다 — 씬 비용은 그대로이고 늘어난 것은 파일
비용뿐이다. 다시 구울 때마다 2.3MB 가 다시 쓰인다는 점은 감수한다. 더 줄이려면 벽도
합쳐야 하는데, 그러면 벽 하나를 손으로 옮기는 정상 작업이 불가능해진다.

**남은 일.** `SampleScene` 을 구운 레벨로 갈아 끼우는 것. 코드는 준비됐지만
(`BakedMapSource` 가 `ILevelQuery` 와 `INetworkMapSource` 를 모두 구현한다) **플레이해
보지 않고는 확인할 수 없는 작업**이라 여기서 멈췄다. 순서는 ①**Tools ▸ NV ▸ Map ▸
Map Generator** 에서 Backrooms Bake → ②씬의 `Backrooms` 오브젝트를 구운 프리팹으로
교체 → ③`Tools ▸ NV ▸ Map ▸ Export Map Collision` 이 "내용이 같아 쓰지 않았다" 를
말하는지 확인 → ④매치 한 판(열쇠·문·체인 드래그·프리즈 x-ray)과 NavMesh 재베이크.
