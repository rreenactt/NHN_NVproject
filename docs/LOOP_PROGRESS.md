...
| IG-022 | 클라이언트 예측에 `Frozen` 반영 + 플래그 필터 | TODO | P2 | IG-010 | R-1.6 |
...
- 상태: TODO
- 범위: `NetworkBootstrap` 의 리컨실리에이션에서 (a) `EntityFlags.Frozen` 이 서 있으면 로컬
  예측도 이동을 멈추고, (b) 예측 상태와 서버 스냅샷을 비교할 때
  `StateProjection.SimulationFlagsOf` 로 매치 비트를 걸러낸다(IG-009 가 준비해 두었다).
- 왜 IG-010 에서 뺐는가: 리컨실리에이션은 잘못 건드리면 이동 전체가 깨지고, 그 검증은 전문
  적용과 다른 축이다(움직임이 부드러운가 vs 시계가 맞는가). 한 커밋에 섞으면 어느 쪽이 원인인지
  가릴 수 없다.
- 증상(고치지 않으면): 서버는 잠금을 입력 무력화로 구현하므로 클라이언트가 자기 입력으로 계속
  예측하고 매 틱 되돌려진다 — **역할 공개 4초 동안 화면이 떨린다.**
...
**IG-022 — 클라이언트 예측에 `Frozen` 반영** (P2) — 남은 TODO 중 가장 앞이다.
