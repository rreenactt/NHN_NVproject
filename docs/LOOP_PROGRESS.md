...
### 3. 남은 태스크 11개 — 전부 사람의 결정이나 사람의 손을 기다린다

| 태스크 | 상태 | 무엇을 기다리는가 |
|---|---|---|
| IG-007 승리 조건 | BLOCKED | **OQ-2 + OQ-6** (최우선) |
| IG-021 클라이언트 판정 경로 제거 | BLOCKED | IG-007 |
| IG-013 장치 6종 → IG-015 파괴 | BLOCKED | **OQ-1** |
| IG-016 체인 견인 → 재장전 | BLOCKED | **OQ-4** |
| IG-017 근접 보이스 | BLOCKED | **OQ-3** |
| IG-020 레거시 파일 정리 | BLOCKED | **OQ-5** |
| IG-031 탈출·피격 타이브레이크 | BLOCKED | **OQ-8** (현재 동작은 `TieBreakTests` 가 고정) |
| IG-023 클라이언트 예측 | TODO (P4) | **실측.** AS-8 이 "예측이 없다" 를 기록했고 §8 은 요구하지 않는다 — 로컬 서버에서는 증상이 없어 손대면 무엇을 고쳤는지 알 수 없다 |
| IG-022 예측에 `Frozen` 반영 | DEFERRED | IG-023 |
