...
**IG-024 — `MatchManager` 의 씨드 계산 중복 정리** (P4). 작고 명확하며 IG-011c3 이후로 죽은
경로일 가능성이 높다 — 씨드가 와이어에서 빠졌으므로 서버에서 오는 씨드 계산이 남아 있다면 그것은
쓰이지 않는 코드다. **먼저 확인하고, 죽었으면 지우고, 살아 있으면(오프라인) 왜 두 곳인지 적는다.**

그 다음: IG-026(E 키 통합, 오프라인 게이트 일치) → IG-027(열쇠 흩뿌리기) → **IG-028b**.
