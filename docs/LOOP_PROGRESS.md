...
**IG-027 — 사망 시 흘린 열쇠를 흩뿌리기** (P4). 남은 것 중 가장 작다.

IG-014b 는 흘린 열쇠를 **사망 지점 한 점**에 놓는다. 규칙("사망 지점에 흘린다")에는 맞지만 한
무더기는 밟은 Runner 가 한 틱에 전부 줍고 시각적으로도 하나처럼 보인다. 흩뿌릴 반경이 기획서에
없으므로 **AS 로 기록하고 진행**한다(게임플레이 영향이 작다 — 회수 난이도가 조금 오른다).
클라이언트의 `ScatterKeys` 가 쓰던 값을 참조한다.

그 뒤 남는 것은 **IG-028b(발사 이벤트) 하나**이고, 그것이 진행 가능한 것 중 유일하게 설계 판단을
요구한다 — 새 `EventKind` 는 이 프로젝트가 피해 온 **"전문이 아니라 알림"** 을 만드는 일이므로
**ADR 초안부터 쓴다.** 그 뒤로는 OQ 의 답이 없으면 §10 의 질문 리포트 상태로 돌아간다.
