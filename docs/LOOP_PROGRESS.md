...
- 범위: `KeyPickup`·`EscapeDoor`·`MapDevice`·`ChainAltar` 가 **좌표를 받아 그려지는 것**이 된다.
  ...
  `IsFreeFloor`·`ScatterKeys` 가 클라이언트에서 사라진다.
- **오프라인 연습 모드를 어떻게 할지 결정해야 한다.** 배치 코드가 사라지면 세션 없이 Play 할 때
  목표물이 하나도 생기지 않는다. 계획서(§5.2 끝)는 배치 함수를 `Shared` 에 두어 양쪽이 쓰되
  좌표만 와이어로 내려보내는 방식을 권한다 — 씨드가 와이어에 없으면 정보 누출 없이 코드 한 벌을
  유지할 수 있다. 지금 배치는 `Modules/Realtime`(서버 전용)에 있으므로, 그 선택을 하려면
  `MatchRules.PlaceObjectives` 를 `Shared` 로 옮겨야 한다 → **IG-011c 시작 시 판단.**
