...
| IG-027 | 사망 시 흘린 열쇠를 흩뿌리기 (표현) | TODO | P4 | IG-014b | R-6.7 |
...
- 상태: TODO (P4)
- IG-014b 는 흘린 열쇠를 **사망 지점 한 점**에 놓는다. 클라이언트의 `ScatterKeys` 는 반경을 두고
  퍼뜨리는데 그 반경이 기획서에 없고, 규칙은 "사망 지점에 흘린다" 이므로 한 점이 규칙에 맞다.
- 다만 한 무더기는 밟은 Runner 가 한 틱에 전부 줍고(습득 판정이 모든 열쇠를 훑는다) 시각적으로도
  하나처럼 보인다. 퍼뜨릴 반경을 정하려면 `MatchConstants` 에 값이 하나 더 필요하다 →
  **게임플레이 영향이 작은 값이므로 AS 로 진행 가능하지만, 지금 할 이유가 없다.**
...
**IG-014c — 클라이언트 전투 적용 + 로컬 판정 차단** (P2). **이것이 R-3.1 을 닫는다.**

서버가 총알을 날리고 맞히고 쓰러뜨린다. 그런데 클라이언트는 여전히 **자기 총알로 자기가 판정**한다
(`Bullet` → `SendMessageUpwards("OnHit")` → `PlayerAgent.OnHit` → `MatchManager.ReportHit`). 즉
지금은 두 심판이 도는 상태이고, 그중 하나가 **"내가 맞췄다" 를 클라이언트가 주장하는 구조**다 —
갭 매트릭스가 "가장 심각하다" 고 적어 둔 바로 그 경로다.

할 일: `EntityFlags.Bleeding`·`Downed` 를 받아 몸에 적용(IG-012c2 의 `ApplyEscaped` 와 같은 형태),
`MatchParticipant.Hits` 로 부상 표시, `Bullet` 을 순수 표현으로, `ReportHit` 를
`ServerOwnsObjectives`… **가 아니라 새 플래그가 필요하다** — 전투는 목표물이 아니므로 이름이
맞지 않는다. `ServerOwnsCombat` 을 따로 두거나 둘을 아우르는 이름으로 바꿀지 판단해야 한다.

**남은 판단 하나: 서버 총알을 그릴 것인가.** 안 그리면 남이 쏜 총알이 보이지 않고(지금은 각자
자기 총알만 그린다), 그리려면 스냅샷에 총알을 실어야 한다 — 별도 태스크(IG-028)가 될 수 있다.
탄약 HUD 도 `MatchParticipant` 에 자리가 없어(`Hits` 는 있고 탄약은 없다) 같은 판단에 걸린다.
