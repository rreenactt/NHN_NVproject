...
| IG-028b | 발사 이벤트로 서버 총알 그리기 | TODO | P3 | IG-028a | R-3.1 (연출) |
...
**IG-026 — `PlayerInteractor` 의 E 키를 `ConsumeInteract` 로 통합** (P4).

지금 E 키를 두 곳이 읽는다 — 와이어 경로는 `FirstPersonController` 의 래치를 소비하고, 오프라인
경로는 `PlayerInteractor` 가 `wasPressedThisFrame` 을 직접 본다. 래치를 소비하는 것이 와이어뿐이라
신호를 먹지는 않지만 **`MovementLocked` 게이트가 와이어 경로에만 있다** — 오프라인에서는 체인에
끌려가는 중에도 장치를 쓸 수 있다. 합치면 두 경로가 일치한다.

**주의: 오프라인 동작을 바꾸는 변경이다**(§6.3). 지금까지의 정리 태스크와 달리 게이트가 하나
늘어나므로, 그 게이트가 옳은지 먼저 확인한다 — 체인 벌칙 중 장치 사용이 막혀야 하는가는
기획서 §4.3 의 "행동불가" 해석에 달려 있고, 애매하면 §6.4 로 OQ 에 올린다.

그 다음: IG-027(열쇠 흩뿌리기) → **IG-028b**(발사 이벤트, ADR 필요).
