...
| IG-024 | `MatchManager` 의 시드 계산 중복 정리 | TODO | P4 | IG-011c3 | (정리) |
...
| IG-014b | 피격 판정 (출혈·순간이동·사망) | TODO | P2 | IG-014a | R-3.1, R-3.3, R-3.4, R-6.7 |
| IG-014c | 클라이언트 전투 적용 + 로컬 판정 차단 | TODO | P2 | IG-014b | R-3.1, R-2.1 |
...
| IG-025 | `Jump` 엣지가 입력 반복에 실리는 문제 | TODO | P4 | - | (품질) |
...
- IG-012b2 에서 발견했다. 새 입력이 없으면 서버가 마지막 입력을 반복하는데
  (`Room.StepPlayer`) 그 프레임에 `Jump` 가 남아 있다. `PlayerMovement.Step` 이 접지 상태로
  걸러 대부분 무해하지만, **착지하는 틱에 반복이 아직 살아 있으면 재점프가 성립한다.**
- 상호작용은 `WithoutEdgeButtons` 로 지웠으나 점프는 일부러 건드리지 않았다 — 이동 동작을
  바꾸는 변경이고, 지금 고치면 이 태스크의 검증 범위를 넘는다.
...
- 상태: TODO
- 기획서 근거: §4.1
- 계획: `StepProjectiles` 의 선분을 플레이어 박스와도 교차시킨다(`Raycaster.RayAabb`, 한 박스
  버전이 이미 공개돼 있다). 자기 총알에는 맞지 않는다(`OwnerId`). Runner 만 대상.
  `hitImmunity` 0.75초를 **틱으로** 올린다(22틱 — 0.75×30 = 22.5 이므로 IG-014a 와 같은 올림 판단).
  1방 → `EntityFlags.Bleeding` + `TryRandomFreeFloor` 로 순간이동, 2방 → 사망 + 소지 열쇠를
  그 자리에 흘리기(`Objectives.AddKey`). `MatchParticipant.Hits` 를 채운다.
- **되감기(lag compensation)는 넣지 않는다.** 발사체가 실체이므로 비행에는 필요 없고, 되감기가
  필요한 것은 발사 순간의 **사수** 위치뿐이다 — 지금은 서버가 자기 시뮬레이션의 눈 위치에서
  쏘므로 그 문제가 없다. 200ms 상한 이력은 클라이언트 예측(IG-023)이 들어올 때 필요해진다.

### IG-014c — 클라이언트 전투 적용 + 로컬 판정 차단
- 상태: TODO
- 기획서 근거: §4.1, §2.1
- 계획: `Bullet` 을 순수 표현으로 만들고(`SendMessageUpwards("OnHit")` 제거),
  `PlayerAgent.OnHit`·`MatchManager.ReportHit` 를 세션이 있을 때 막는다. 출혈·사망은
  `EntityFlags` 와 `MatchParticipant.Hits` 로 받는다. **이것이 R-3.1 의 구멍을 닫는다** —
  "내가 맞췄다" 를 클라이언트가 주장하는 마지막 경로다.
- 서버 총알을 그릴지 결정해야 한다. 안 그리면 남의 총알이 보이지 않고, 그리면 스냅샷에 총알을
  실어야 한다 — 별도 태스크가 될 수 있다.
...
**IG-014b — 피격 판정(출혈·순간이동·사망)** (P2).

총알이 서버에서 날아가지만 **아무도 맞지 않는다.** IG-014b 가 선분을 플레이어 박스와 교차시키고,
그것이 R-3.1("클라이언트가 내가 맞췄다고 주장하는 경로")을 닫는 마지막 판정이다.

준비된 것: `Raycaster.RayAabb` 의 한 박스 버전이 이미 공개돼 있고, `Projectile.OwnerId` 로 자기
총알을 걸러낼 수 있고, `EntityFlags.Bleeding` 자리가 IG-009 에서 만들어져 있고,
`MatchParticipant.Hits` 가 비어 있다. **순간이동 착지점은 `TryRandomFreeFloor` 가 답한다** —
AS-7 이 셀 중심을 돌려주기로 한 이유가 여기서 값을 한다(착지점은 플레이어다).

첫 작업은 `hitImmunity` 0.75초를 틱으로 옮기는 것이고, **0.75 × 30 = 22.5 라 IG-014a 와 같은
올림 판단(23틱)이 필요하다** — 무적 창은 길수록 안전한 쪽이다(한 버스트가 순간이동을 관통해
죽이는 것을 막는 장치이므로).

### 이번에 확인한 것 (아래 원래 계획과 다른 점)

계획서는 "스윕 판정을 만든다" 로 적혀 있었지만 **`Shared` 에 이미 있었다** — `Raycaster.RayAabb`
와 `CollisionWorld.Raycast` 가 테스트까지 갖춘 채로 아무도 부르지 않고 있었다. 만들 것은
호출자뿐이었다. 계획을 그대로 믿고 시작하면 있는 코드를 두 번 만들었을 것이다.
