...
| IG-028 | 서버 총알을 스냅샷에 실어 그리기 | TODO | P3 | IG-014c | R-3.1 (연출) |
...
- 탄약 HUD 도 여기 걸린다 — `MatchParticipant` 에 탄약 자리가 없다(`Hits` 는 있다).
...
**답이 필요하지 않은 것도 있다.** IG-018(EditMode 인프라)·IG-023~028 은 진행 가능하고, 그중
**IG-028(서버 총알·탄약 와이어)이 가장 값이 크다** — 지금 남이 쏜 총알이 화면에 없다.

**그리고 답보다 먼저 해야 할 것이 하나 있다:** 아래 "사람이 해야 하는 검증" 의 2클라이언트 스모크가
**한 번도 실행되지 않았다.** 서버 판정은 394개 테스트로 고정됐지만 그것이 화면에 도달하는지는
...
**리포트는 썼다(이터레이션 27). 루프가 계속 돌면 IG-028 을 집는다.**

**IG-028 — 서버 총알·탄약을 와이어에 싣기** (P3). 진행 가능한 것 중 가장 값이 크다:
- 지금 각자 **자기 총알만** 그린다 → 남이 쏜 예광탄이 보이지 않는다
- 히트마커가 로컬 총알로 뜨므로 **서버 판정과 어긋날 수 있다**
- 탄약 HUD 가 로컬 값을 그린다(R-9.3) — 판정은 서버인데 표시는 클라이언트다

설계 판단: **총알 상태를 매 틱 싣지 않는다.** 8인 룸에서 32발은 대역폭이 크고, `EntityFlags` 는
이미 8비트를 다 썼다. **발사 이벤트(시작점·방향·틱)만 보내고 비행은 클라이언트가 재현**하는 편이
훨씬 싸다 — 총알은 등속 직선이고 중력이 0 이므로 재현이 정확하다. 탄약은 `MatchParticipant` 에
바이트 하나를 더하면 되지만 **`WireSize` 5B 가 6B 가 되므로 `WireSizeTests` 가 잡을 것이고,
프로토콜 버전을 올릴지 판단해야 한다**(크기가 바뀌므로 이번에는 올려야 한다).

그 뒤 순서: IG-024(씨드 중복, 작다) → IG-025(Jump 엣지) → IG-026(E 키 통합) → IG-027(열쇠 흩뿌리기).
