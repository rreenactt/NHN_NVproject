...
| IG-012c2 | 클라이언트 탈출 적용 | TODO | P2 | IG-012c1 | R-6.7 |
...
- 범위에서 **일부러 뺀 것들** — 각각 지금 적용하면 퇴화한다:
  ...
  | 뺀 것 | 이유 |
  |---|---|
  | ~~`escapes`~~ | **IG-012c1 부터 서버가 센다.** 적용은 IG-012c2 이고, 그때 로컬 `TickEscapes` 도 함께 막아야 한다 |
...
- 상태: TODO
- 계획: `MatchState.escapes` 를 적용하고(`AcceptEscapes`), `EntityFlags.Escaped` 를 받은 몸을
  감춘다. 로컬 `TickEscapes`·`ReportEscape` 를 `ServerOwnsObjectives` 에서 막는다.
  IG-012b3 와 같은 형태다.
- 주의: `escapes` 는 **Seeker 사본에도 실제 값이 온다.** 열쇠 진행도와 달리 거르지 않으므로
  `MatchSync` 의 "적용하지 않는다" 표에서 그 행을 지울 때 역할 조건을 붙이지 않는다.
...
| IG-012c1 | **IG-012c2 가 끝난 뒤에 확인한다.** 문이 열린 뒤 문간에 0.8초 서 있으면 그 Runner 가 사라지고, **두 화면(Seeker 포함)의 탈출 수가 함께 오른다** — 열쇠 진행도와 달리 이 값은 Seeker 도 받는다 |
...
**IG-012c2 — 클라이언트 탈출 적용** (P2). IG-012b3 와 같은 형태이므로 새 설계 판단은 없을 전망이다.

서버가 탈출을 세고 `Escaped` 플래그를 보낸다. 클라이언트는 `escapes` 를 적용하지 않고 로컬
`TickEscapes` 로 세고 있으므로, 지금 **두 심판이 병행하는 상태**다 — b2/b3 사이와 같다.

**IG-018 은 이번에 앞으로 당기지 않았다.** 조사해 보니 Unity 의 asmdef 는 **`Assembly-CSharp` 를
참조할 수 없다** — 테스트 asmdef 에서 클라이언트 스크립트를 보려면 `Assets/Scripts` 전체를 asmdef
로 옮겨야 하고, 그것은 MCP 커맨드 스크립트와 에디터 스크립트의 참조까지 흔드는 큰 변경이다.
게다가 D-2 가 이미 "순수 로직 검증은 `dotnet test`" 로 결정해 두었다. **IG-018 을 하려면 그 asmdef
이관을 먼저 태스크로 세워야 하며, 그 전에 ADR 이 필요하다** — 다음에 IG-018 을 열 때의 첫 단계다.
