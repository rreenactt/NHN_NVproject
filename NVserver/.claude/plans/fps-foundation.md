...
| 상태 | **완료 (변경됨)**. `Client/` 를 새로 만들지 않고 기존 Unity 프로젝트 `../NVproject` 를 클라이언트로 채택했다 — 게임(블록 캐릭터, 절차적 애니메이션, Backrooms 레벨)이 이미 거기 있었고 두 번째 프로젝트는 그것의 복제였다. `NVproject/Packages/manifest.json` 에 `"com.nv.shared": "file:../../NVserver/Shared"` 를 등록하고 `packages-lock.json` 에 로컬 패키지로 해석된 것을 확인했다. `NVserver/Client/` 는 제거했다 |
...
| 상태 | **코드 완료, Unity 측 실행 확인 남음**. 전송 구현과 `.jslib` 는 `NVproject/Assets/{Scripts/Net,Plugins/WebGL}` 로 이관했고 `NetworkClient` 가 접속·Welcome·스냅샷 디코드·30Hz 입력 송신을 담당한다. 같은 `Shared` 코덱을 쓰는 콘솔 클라이언트 두 개로 서버측 왕복을 검증했다(플레이어 0·1 배정, 스냅샷 30Hz, 서버 권위 이동과 벽 정지). 에디터 Play 와 WebGL 빌드 확인은 사람 몫이다 |
...
| 상태 | **완료**. 레벨이 코드 생성이라 콜리전 출처도 생성기로 잡았다 — `INetworkMapSource` 를 구현한 레벨의 콜리더 박스를 `MapExport` 가 `MapData` 로 옮기고, 에디터 메뉴가 `MapData/{맵이름}.json` 으로 쓴다. 런타임에는 같은 함수로 해시를 계산해 `Welcome.MapHash` 와 대조하고 불일치를 에러로 남긴다. `backrooms.json`(박스 1367개, 해시 9F23B712)과 `test-room.json`(박스 10개, 해시 27A9412D)을 사전 생성해 두었으므로 서버는 바로 뜬다. 씨드·격자를 바꾼 뒤에는 에디터 export 가 권위 있는 경로다 |
...
| 담당 | 사람 |
| 상태 | **코드 완료, 조작 관찰 남음**. `FirstPersonController` 에 서버 권위 모드를 넣어 `CharacterController` 를 움직이지 않고 입력 샘플링과 시점 회전만 남겼다. 스냅샷 위치를 그대로 적용하되 30Hz 계단을 펴기 위해 로컬은 짧게 감쇠한다 — 걸음 속도를 실제 변위에서 재는 구조라 계단을 그대로 적용하면 다리가 덜컥거린다. 원격 플레이어는 T34 의 100ms 보간 버퍼를 이미 쓴다 |
...
| 담당 | 사람 |
| 상태 | **코드 완료, 시각 확인 남음**. `SnapshotBuffer` 가 로컬 도착 시각 기준으로 100ms 과거를 보간한다 — 서버 틱 기준으로 잡으면 시계 동기가 필요하고, 도착 시각 기준은 지터를 그대로 흡수하며 스냅샷이 하나 빠져도 구간이 길어질 뿐이다. 원격 몸은 로컬과 같은 리그·같은 애니메이터를 쓰고 이동 입력은 변위에서 역산한다 |
| 선행 조정 | M5(T32·T33) 보다 먼저 들어갔다. 원격 플레이어가 보이지 않으면 멀티플레이 확인 자체가 불가하고, 로컬 예측과는 코드 경로가 겹치지 않는다 |

### T36 — 멀티플레이 테스트 씬과 테스트 룸 (추가)

| 항목 | 내용 |
|---|---|
| 목적 | 다중 접속 확인을 실제로 가능하게 한다. Backrooms 180m 미로에서는 두 클라이언트가 서로를 찾는 데 시간을 다 쓰고, 찾으려던 증상(떨림·고무줄·미끄러짐)은 상대가 화면에 있어야만 보인다 |
| 선행 | T30, T34 |
| 변경 대상 | `NVproject/Assets/Scripts/TestRoomMap.cs`, `Assets/Editor/MultiplayerTestScene.cs`, `MapData/test-room.json` |
| 내용 | 40m 정사각 방, 벽 4면, 중앙 플랫폼(올라설 지형), 엄폐물 4개, 스폰 8개가 반지름 15m 링에서 중앙을 본다. 안개 없음 — 방이 어디까지인지 안 보이는 것이 Backrooms 의 목적이지만 여기서는 증상을 가린다. 씬 생성은 에디터 메뉴가 조립하고 저장한다 |
| 결정 — 수치 | 박스 좌표를 정확히 표현되는 수(절반·1/4)로만 잡았다. 콜리전 박스는 해시되어 서버 값과 대조되므로, 왕복이 정확한 값은 "해시가 왜 다른가" 한 부류를 없앤다. 난수도 쓰지 않으므로 사전 생성한 JSON 과 Unity 계산이 반드시 일치한다 |
| 완료 조건 | 서버를 `test-room` 으로 띄우고 두 클라이언트가 서로를 본다 |
| 검증 | 콘솔 클라이언트 두 개로 스폰 좌표·벽 정지 확인 완료 (스폰 (0,0,15)·(10.61,0,10.61), +Z 벽 안쪽 면 20m 에서 반지름 0.4 를 빼고 19.6m 에 정지). 에디터 Play 로 시각 확인은 사람 몫 |
| 담당 | 에이전트 (코드) + 사람 (씬 생성 메뉴 실행, 조작 관찰) |
