...
- **슬롯 재활용 위험**: 정적 룸은 매치 중 입장이 가능하고 슬롯이 재사용되므로, 나간
  술래의 번호를 새 참가자가 받으면 `MatchFlagsFor` (`Room.cs:1053`) 가 그 사람에게
  `EntityFlags.Seeker` 를 붙인다 — 이전 커밋의 코드 리뷰가 지적한 잠재 결함이고,
  이 작업이 그 창을 함께 닫는다(매치가 유예 안에 끝나므로).
...
- **남은 사람이 무슨 일이 일어났는지 볼 시간을 준다.** "술래가 나갔고 곧 끝난다" 를
  화면이 말할 수 있다(§4).
- **유예 중 회복되면 취소한다.** 초대 코드 룸에서는 사실상 일어나지 않지만(새 세션 =
  새 슬롯), 판정이 구조적이므로 취소 규칙도 공짜다 — 매 틱 재평가해서 진행 가능해지면
  데드라인을 지운다. 봇 채우기(`TopUpBots`)는 대기 단계에만 돌므로 매치 중 봇이
  빈 자리를 메우는 경로는 없다.
...
| 정적 룸, 나간 술래의 슬롯을 새 참가자가 받음 | 유예(5초) 안에 같은 슬롯을 받고 명단에 들어오면 구조상 "Seeker 있음"으로 회복된다 — 드물지만 판정이 구조적이므로 일관된다. 그 사람은 자기가 술래인 채 매치 중간에 서 있게 되는데, 이는 §1.1 의 기존 슬롯 재활용 문제이며 이 작업은 그 노출 시간을 480초에서 5초로 줄인다. 완전한 해결(매치 중 입장 거절 또는 즉시 중단)은 별도 과제로 기록 |
