...
**초대 코드를 채팅으로 주고받는 게임이라 링크 카드는 사실상 첫인상이다.** 태그가 없으면 카카오톡·디스코드·슬랙이 문서 제목을 그대로 퍼 가고, 기본 템플릿에서 그 제목은 `Unity Web Player | NVproject` 였다.

`og:` + `twitter:` 태그를 넣고 카드 이미지 `TemplateData/social-card.png`(1200×630)를 템플릿에 함께 둔다. 그림도 레벨 팔레트로, 어두운 복도 끝에 문 하나.

**주소를 절대 경로로 박는 것은 타협이 아니라 요구다.** 크롤러는 상대 경로 `og:image` 를 신뢰성 있게 풀지 않고, **자바스크립트를 실행하지 않으므로** 런타임에 `location` 을 읽어 채우는 방법이 없다. 그래서 배포 도메인 `https://play.nhn-backroom.kro.kr/` 를 템플릿에 적는다 — 이 값은 `Caddyfile` 의 play 블록, `appsettings.Production.json` 의 CORS 허용 오리진과 **한 벌**이고 도메인이 바뀌면 셋이 같이 바뀐다. 로컬 빌드에서는 이 태그가 틀린 주소를 가리키지만 로컬 빌드의 링크는 아무도 공유하지 않으므로 대가가 없다.

제목은 `{{{ PRODUCT_NAME }}}` 매크로에서 뽑는다. `<title>` 과 같은 값이어야 하고, 이름을 바꿨을 때 카드만 옛 이름으로 남는 것이 가장 흔한 어긋남이다.

알아 둘 대가 하나: **파일명에 해시가 없다.** 카드 그림을 나중에 바꾸면 크롤러(카카오·디스코드)가 캐시해 둔 옛 그림이 한참 남는다. 각 플랫폼의 디버거로 캐시를 비우는 것 외에 방법이 없다.
