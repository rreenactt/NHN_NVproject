...
`Room.Advance()` 는 매 틱 이 순서로 돈다. **순서 자체가 규칙이다** — 아래 표는
코드에서 읽은 것이고, 바꾸면 규칙이 바뀐다.

| # | 단계 | 하는 일 |
|---|---|---|
| 1 | `DrainCommands` | 방 상태를 바꾸는 요청(참가·준비·시작·강퇴 …). HTTP·소켓 스레드가 큐에 넣은 것 |
| 2 | `DrainInputs` | 세션별 입력 프레임 수신 |
| 3 | `StepPlayer` | 입력 검증 → 이동. 사람과 봇이 같은 함수를 지난다 |
| 4 | `PickUpKeys` | 열쇠 습득 |
| 5 | `InsertKeys` | 문에 삽입 |
| 6 | `TickEscapes` | 문간 유지 판정 |
| 7 | `StepProjectiles` | 날아가던 총알을 진행시키고 명중을 판정 |
| 8 | `FireWeapons` | 이번 틱의 발사 요청 처리 |
| 9 | `ProjectWire` | 몸의 와이어 표현을 다시 만든다 — **틱의 마지막 단계** |
| 10 | 매치 시계 · 중단 감시 | 시간 종료 판정, 인원 부족의 유예 데드라인 |

두 가지가 순서에 의존한다.

**`ProjectWire` 가 마지막인 이유**는 목표물 판정(4~6)이 세우는 플래그가 **같은 틱의
스냅샷**에 실려야 하기 때문이다. 이동 직후에 만들면 그 틱의 출혈·탈출·쓰러짐이 한
틱 늦게 나간다.

**6 과 7 의 순서는 미결 사항이다(OQ-8).** 지금은 탈출 판정이 총알보다 앞서므로,
유지 시간의 **마지막 틱에 도착한 총알은 탈출을 끊지 못한다.** 그런데
`EscapeHoldTime` 의 문서화된 목적은 "술래가 끊을 수 있는 순간을 만드는 것" 이다.
창은 한 틱(33ms)이지만 목표 탈출 수가 작아 결과는 이진이다 — 밸런스 결정이므로
`docs/LOOP_PROGRESS.md` 의 OQ-8 로 남아 있다.
