...
> **진행 상황.** Phase 0~5 를 구현했다. Phase 4-3(브로드페이즈)은 **재 보고 하지 않기로
> 했다** — 측정값이 추정과 한 자리 이상 달랐다. §7 의 각 단계와 §10 을 본다.
>
> **P8·P9 의 비용 추정은 틀렸다.** 상한을 실제 값처럼 읽었고, 실측은 그 1/100 이다. 정정을
> §10 에 적었다. 진단(중복 계산이 접속 프레임에 있다, 브로드페이즈가 없다)은 사실이고,
> 그것이 얼마나 비싼지에 대한 판단만 틀렸다.

아래의 모든 진단은 리포지터리의 코드와 실제 맵 파일을 읽어서 확인한 것이며, 근거를
`파일:줄` 로 적었다.
...
1. **했다 — 씬 Collider 포함 + 회전 거절 (B3), P12.** 계획서는 이것을 "해시가 바뀐다" 로
   분류했는데 **틀렸다.** 표시된 볼륨이 씬에 하나도 없으면 박스 목록이 그대로이므로 기능은
   쓰이기 전까지 무해하고, 지금 두 씬에는 볼륨이 없어 해시가 바뀌지 않는다. 재-export 를
   요구하지 않고 넣을 수 있었다.
2. **하지 않는다 — 박스 목록 정렬 (B4).** 유일한 근거가 §6.5 의 브로드페이즈였고(박스를 격자
   셀 순으로 정렬하면 버킷을 인덱스 구간으로 표현할 수 있다) 그것을 §10 에서 취소했다. 남는
   이득은 "생성 순서가 흔들려도 해시가 같다" 인데, 생성 순서는 이미 씨드에 결정적이므로 흔들리지
   않는다. 값이 없는 채로 모든 맵의 해시를 바꾸는 변경이 된다.
3. **했다 — 배치 export + 씬↔맵 표 단일화 (A6).** 표를 `MapSceneTable` 하나로 옮기고 세션
   라우터와 배치 export 가 같은 것을 읽는다.
...
Phase 4-3 의 착수 조건은 §9-4 가 정한 "측정 먼저" 였다. 쟀고, **추정이 한 자리 이상 틀렸다.**

Release 빌드, `backrooms.json`(736박스 / 2450셀):
...
**무엇을 잘못 읽었나.** P8 은 `MarkFreeFloor` 를 "2450셀 × 4회 반복 × 736박스 ≈ 720만 회" 로
적었다. 그 곱은 **상한**이고 실제 경로가 아니다 — 겹치지 않는 자리에서는 첫 패스가 `resolved`
를 참으로 두고 끝나므로 반복이 1회이며, `Aabb.Overlaps` 는 비교 여섯 번이다. P9 의 "초당 17만 회
순회" 도 같은 방식으로 계산했고, 그 순회가 실제로는 예산의 0.3% 다.

**결정.**

- **Phase 4-3(브로드페이즈)을 하지 않는다.** 값이 작고 위험이 크다. `Depenetrate` 는 박스를
  순차적으로 밀어내므로 순회 순서가 결과를 바꾸고, 한 패스 중에 위치가 움직여 처음에는 겹치지
  않던 박스와 겹칠 수 있다 — 후보 집합을 미리 정하는 방식으로는 그 경우에 지금과 비트가 같은
  결과를 보장할 수 없다. 어긋나면 클라이언트 예측과 갈리고, 증상은 "특정 위치에서만 캐릭터가
  튐" 뿐이다. `CollisionWorld` 의 낡은 주석("박스 수십 개 규모")을 이 측정값으로 바꿨다.
- **Phase 4-1(캐시)은 남긴다.** 다만 이유가 달라진다. 접속 프레임에서 없어진 비용의 대부분은
  `MarkFreeFloor`(1.3 ms)가 아니라 **`ComputeCollision` 의 레벨 재-해결** 이다 — 미로를 씨드에서
  다시 풀고 벽 병합을 다시 하는 경로이고, 그것을 접속하는 프레임에 두는 것은 그 자체로 잘못된
  자리다. 캐시가 없앤 것은 "같은 레벨을 두 곳에서 각각 다시 푸는 것" 이다.

**남는 규칙.** `O(N)` 이 보이면 재고 나서 고친다. 상한을 실제 비용으로 읽으면 위험만 사고
얻는 것이 없는 변경을 하게 된다. 박스 수를 크게 늘리는 변경(층 추가, 씬 프랍 포함)을 할 때
위 표를 기준선으로 다시 잰다.
