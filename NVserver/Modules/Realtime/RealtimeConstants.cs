...
namespace NV.Realtime
{
    ...
    internal static class RealtimeConstants
    {
        ...
        internal static class Rooms
        {
            ...
            ///
            /// **8 이었다. 5 로 내렸다.** 이 값이 정하는 것은 버퍼 크기만이 아니다 — 한 매치에
            /// 술래 하나와 도망자 나머지가 서니, 정원은 곧 **도망자 수**이고 그것이 균형이다.
            ///
            /// 룸마다 다른 정원을 두지 않는다. 정원은 서버가 판정하는 값이고, 맵 파일의 권장
            /// 인원(`MapData.Meta`)은 표시용이다 — 편집할 수 있는 파일이 규칙을 정하면 안 된다.
            /// 클라이언트도 이 값을 복제하지 않고 `GET /rooms/{code}` 로 받는다.
            ///
            /// 캐릭터 수(`ProtocolInfo.LobbyCharacterCount`, 8)와는 **무관하다.** 우연히 같았을
            /// 뿐이고, 지금은 다르다 — 다섯 명이 여덟 벌 중에서 고른다.
            ...
            /// 맵마다 자동으로 여는 테스트 룸의 id 앞자리. `test-{맵 id}` 다.
            ///
            /// 소문자·하이픈이라 룸 id 규칙 안에 있다. 초대 코드가 같은 문자열을 뽑는
            /// 경로는 산술적으로 남아 있지만, 그 충돌은 코드 생성의 `TryAdd` 재시도가
            /// 이미 흡수한다 — 정적 룸이 항상 먼저 등록되기 때문이다.
            public const string PerMapRoomPrefix = "test-";
