...
namespace NV.Realtime
{
    ...
    internal static class RealtimeConstants
    {
        ...
        internal static class Match
        {
            ...
            /// 매치가 정상 진행 불가(술래 부재, Runner 부재)로 판정된 뒤 종료까지의
            /// 유예. 30Hz 기준 5초다.
            ///
            /// 재접속 대기가 아니다 — 계정이 없어 끊긴 사람과 새로 온 사람을 구분할 수
            /// 없고, 새 세션은 새 슬롯을 받아 어차피 술래 자리와 일치하지 않는다. 남은
            /// 사람이 무슨 일이 일어났는지 볼 시간이고, 판정은 매 틱 다시 하므로 유예
            /// 중에 명단이 회복되면 취소된다.
            ///
            /// 클라이언트가 이 값으로 계산하는 것이 없으므로 `Shared` 가 아니라 여기다.
            public const uint AbortGraceTicks = 5 * 30;
        }
