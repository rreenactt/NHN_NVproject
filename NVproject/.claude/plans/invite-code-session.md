...
| 내용 | `TryCreate(mapId)` → 코드 생성(길이는 룸 수에서 유도, 충돌 시 재생성 후 길이 증가) + `hostToken`(16바이트 무작위). `GetOrCreate` 는 정적 룸에만 남기고 **접속 경로는 `TryGet` 으로 바꾼다** — 없는 코드로 붙으면 `404`. 회수: 마지막 참가자 퇴장 시 즉시, 아직 아무도 들어오지 않은 방은 30초. 제거는 틱 루프가 한다 |
...
| 접속했다 끊은 방 | 1초 뒤 조회가 `404`. 마지막 참가자가 나가면 즉시 회수된다 |
| 만들고 붙지 않은 방 | 3초 뒤 조회가 `200`. 30초 유예가 살아 있다 |
...
| 계획 | 실제 | 이유 |
|---|---|---|
| 빈 룸 만료를 단계별로 60초/30초 | 단계로 가르지 않는다 | 전원이 나가면 룸이 스스로 대기 단계로 돌아가므로 "비어 있으면서 진행 중인 룸" 은 존재하지 않는다. 두 번째 기준은 도달할 수 없는 분기였다 |
| 시작 없이 10분이면 만료 | 없앴다 | 사람이 있는 로비를 시간만으로 닫는 규칙이었다. 아무도 들어오지 않은 방은 다른 기준이 이미 잡는다 |
| 빈 룸을 시간이 지나면 만료 | **마지막 참가자가 나가면 즉시** 회수. 아직 아무도 들어오지 않은 방만 30초 유예 | 마지막 사람이 나간 방을 남겨 둘 이유가 없다. 다만 "참가자 0이면 삭제" 를 문자 그대로 적용하면 `POST /rooms` 와 접속 사이(참가자 0)에 모든 방이 사라진다 |
