...
`Event` carries an `EventKind` byte after the opcode and there are three: `RoomState` (2Hz — phase, host, seeker, roster), `MatchState` (2Hz — match phase, clock, keys inserted, escapes, per-player hits and carried keys), `ObjectiveState` (on change + every 5s — altar, door, keys, devices). **`MatchState` and `ObjectiveState` are encoded per session, not per room**: the codec blanks key progress and omits the door block entirely for a Seeker, so the filter cannot be bypassed by a caller. Adding a kind means adding a `case` to the client's `DispatchEvent` — parsing every `Event` as one kind throws on the others.
